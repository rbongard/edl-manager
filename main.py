...
source = config_vars['source']
edl_list_names = config_vars['edl_list_names']
EXP_DAYS = config_vars['expires']
overwrite_original = config_vars['overwrite_original']

# Configure logging
logging.basicConfig(
    filename='ip-addresses-management.log',
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s',
    datefmt='%Y-%m-%d %H:%M:%S'
)

def log_dataframe_rows(df, current_file, log_type="INFO", additional_info=""):
    ...
    if df.empty:
        if additional_info == "":
            log_message = f"File {current_file}"
        else:
...
def main():
    ...
    for edl_list_name in edl_list_names:
        ...
        df = pd.read_csv(edl_list_name, sep="#", header=None, names=['IP Address', 'Comment', 'Date'])
        ...
        if df.empty:
            ...
            #df_active['IP Address'].apply(is_valid_ip)
